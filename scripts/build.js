...
function checkExist(excludes, file) {
    return typeof excludes==="string"?file.indexOf(excludes)===0:excludes.some((exclude)=>file.indexOf(exclude)===0);
}
...
function copyPublicFolder() {
    ...
    fs.copySync(paths.appPublic, path.join(paths.appBuild,assertDir), {
        dereference: true,
        filter: file => {
            if(excludes&&checkExist(excludes,file)){
                return false;
            }
